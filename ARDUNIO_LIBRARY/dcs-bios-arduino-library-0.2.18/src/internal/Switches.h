	...
	class Matrix2Pos : PollingInput {
		...
		private:
			...
			char readState() {
				...
				if ((round(analogRead(pin_)/divisor)) == 2) return 2;
				if ((round(analogRead(pin_)/divisor)) == 3) return 3;
				if ((round(analogRead(pin_)/divisor)) == 4) return 4;
				if ((round(analogRead(pin_)/divisor)) == 5) return 5;
				if ((round(analogRead(pin_)/divisor)) == 6) return 6;
				if ((round(analogRead(pin_)/divisor)) == 7) return 7;
				if ((round(analogRead(pin_)/divisor)) == 8) return 8;
				if ((round(analogRead(pin_)/divisor)) == 9) return 9;
				if ((round(analogRead(pin_)/divisor)) == 10) return 10;
			}
		...
		public:
			AnalogMultiPos(const char* msg, char pin, char numOfSteps, float divisor_) {
				msg_ = msg;
				pin_ = pin;
				divisor = divisor_;
				lastState_ = readState();
